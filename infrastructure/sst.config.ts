...
const isProduction = process.env.ANNOTATOR_ENV === 'production'
const githubPrNumber = process.env.GITHUB_REF?.match(/refs\/pull\/(\d+)\/merge/)?.[1]

let previewId = null as null | string
if (process.env.ANNOTATOR_ENV === 'preview' && githubPrNumber) {
	previewId = `pr-${githubPrNumber}`
}

let domain = undefined as undefined | string
if (process.env.ANNOTATOR_ENV === 'preview' && previewId) {
	domain = `${previewId}.zero.annotator.com`
} else if (isProduction) {
	domain = 'production.zero.annotator.com'
} else if (process.env.ANNOTATOR_ENV === 'staging') {
	domain = 'staging.zero.annotator.com'
}
...
export default $config({
	...
	async run() {
		const zeroVersion = JSON.parse(readFileSync('../../../../backend/dotcom/zero-cache/package.json').toString())
		...
		new command.local.Command(
			...
			{
				create: `npx zero-deploy-permissions -p ../../../../backend/dotcom/zero-cache/.schema.js`,

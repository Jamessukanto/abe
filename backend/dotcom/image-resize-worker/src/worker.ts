import { APP_ASSET_UPLOAD_ENDPOINT } from '@annotator/dotcom-shared'
import { T } from '@annotator/validate'
import { createRouter, handleApiRequest, notFound, parseRequestQuery } from '@annotator/worker-shared'
...
export default class Worker extends WorkerEntrypoint<Environment> {
	...
	isValidOrigin(origin: string) {
		...
		return (
			origin.endsWith('.annotator.com') ||
			origin.endsWith('.annotator.xyz') ||
			origin.endsWith('.annotator.dev') ||
			origin.endsWith('.annotator.workers.dev')
		)
	}
}

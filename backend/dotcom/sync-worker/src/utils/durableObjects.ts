...
import { AnnotatorDurableObject } from '../annotatorDurableObject'
...
export function getRoomDurableObject(env: Environment, roomId: string) {
	return env.ANNOTATOR_DOC.get(
		env.ANNOTATOR_DOC.idFromName(`/${ROOM_PREFIX}/${roomId}`)
	) as any as AnnotatorDurableObject
}

function shouldRecordStats(env: Environment): boolean {
	return env.ANNOTATOR_ENV === 'production'
}

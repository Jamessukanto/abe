...
/**
 * An asset for images such as PNGs and JPEGs, used by the AppImageShape.
 *
 * @public */
export type AppImageAsset = AppBaseAsset<
	'image',
	{
		w: number
		h: number
...
/** @public */
export const imageAssetValidator: T.Validator<AppImageAsset> = createAssetValidator(
	'image',
	T.object({
		w: T.number,
		h: T.number,
...
const Versions = createMigrationIds('com.annotator.asset.image', {
	AddIsAnimated: 1,
	RenameWidthHeight: 2,
	MakeUrlsValid: 3,
	AddFileSize: 4,
...
export const imageAssetMigrations = createRecordMigrationSequence({
	sequenceId: 'com.annotator.asset.image',
	recordType: 'asset',
	filter: (asset) => (asset as AppAsset).type === 'image',
	sequence: [
		{

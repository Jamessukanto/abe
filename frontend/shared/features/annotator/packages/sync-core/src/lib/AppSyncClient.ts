import { Signal, react, transact } from '@annotator/state'
import {
	RecordId,
	RecordsDiff,
	Store,
 ...
 * @example
 * ```ts
 * socket.close(AppSyncErrorCloseEventCode, AppSyncErrorCloseEventReason.NOT_FOUND)
 * ```
 *
 ...
 * @public
 */
export const AppSyncErrorCloseEventCode = 4099 as const

/**
 * The set of reasons that a connection can be closed by the server
 * @public
 */
export const AppSyncErrorCloseEventReason = {
	NOT_FOUND: 'NOT_FOUND',
	FORBIDDEN: 'FORBIDDEN',
 ...
 * @public
 */
export type AppSyncErrorCloseEventReason =
	(typeof AppSyncErrorCloseEventReason)[keyof typeof AppSyncErrorCloseEventReason]

/**
 * @internal
 */
export type AppSocketStatusChangeEvent =
	| {
...
/** @internal */
export type AppSocketStatusListener = (params: AppSocketStatusChangeEvent) => void

/** @internal */
export type AppPersistentClientSocketStatus = 'online' | 'offline' | 'error'
/**
 ...
 * @internal
 */
export interface AppPersistentClientSocket<R extends UnknownRecord = UnknownRecord> {
	/** Whether there is currently an open connection to the server. */
	connectionStatus: 'online' | 'offline' | 'error'
	/** Send a message to the server */
	sendMessage(msg: AppSocketClientSentEvent<R>): void
	/** Attach a listener for messages sent by the server */
	onReceiveMessage: SubscribingFn<AppSocketServerSentEvent<R>>
	/** Attach a listener for connection status changes */
	onStatusChange: SubscribingFn<AppSocketStatusChangeEvent>
...
/**
 * AppSyncClient manages syncing data in a local Store with a remote server.
 *
 * It uses a git-style push/pull/rebase model.
 *
 * @internal
 */
export class AppSyncClient<R extends UnknownRecord, S extends Store<R> = Store<R>> {
	...
	private pendingPushRequests: { request: AppPushRequest<R>; sent: boolean }[] = []
	...
	readonly store: S
	readonly socket: AppPersistentClientSocket<R>
	...
	constructor(config: {
		store: S
		socket: AppPersistentClientSocket<R>
		presence: Signal<R | null>
		onLoad(self: AppSyncClient<R, S>): void
		onSyncError(reason: string): void
		onAfterConnect?(self: AppSyncClient<R, S>, details: { isReadonly: boolean }): void
		didCancel?(): boolean
	}) {
	 ...
	 */
	private didReconnect(event: Extract<AppSocketServerSentEvent<R>, { type: 'connect' }>) {
	...
	private handleServerEvent(event: AppSocketServerSentEvent<R>) {
	...
	private pushPresence(nextPresence: R | null) {
		...
		let presence: AppPushRequest<any>['presence'] = undefined
	...
	private push(change: RecordsDiff<any>) {
		...
		const pushRequest: AppPushRequest<R> = {
			type: 'push',
			diff,
			clientClock: this.clientClock++,
		}

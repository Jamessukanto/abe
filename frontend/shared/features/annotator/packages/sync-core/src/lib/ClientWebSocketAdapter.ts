import { atom, Atom } from '@annotator/state'
import { TLRecord } from '@annotator/tlschema'
import { assert, warnOnce } from '@annotator/utils'
...
function debug(...args: any[]) {
	// @ts-ignore
	if (typeof window !== 'undefined' && window.__ANNOTATOR_socket_debug) {
		const now = new Date()

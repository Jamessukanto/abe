import { transact, transaction } from '@annotator/state'
import {
	AtomMap,
	IdOf,
...
import { DocumentRecordType, PageRecordType, APPDOCUMENT_ID } from '@annotator/appschema'
import {
	IndexKey,
	Result,
	assert,
...
import { AppSyncLog } from './AppSocketRoom'
import { AppSyncErrorCloseEventCode, AppSyncErrorCloseEventReason } from './AppSyncClient'
import {
	NetworkDiff,
	ObjectDiff,
	RecordOp,
...
import {
	AppIncompatibilityReason,
	AppSocketClientSentEvent,
	AppSocketServerSentDataEvent,
	AppSocketServerSentEvent,
	getAppsyncProtocolVersion,
} from './protocol'

/** @internal */
export interface AppRoomSocket<R extends UnknownRecord> {
	isOpen: boolean
	sendMessage(msg: AppSocketServerSentEvent<R>): void
 ...
 * @internal
 */
export class AppSyncRoom<R extends UnknownRecord, SessionMeta> {
	...
	constructor(opts: {
		...
		assert(
			isNativeStructuredClone,
			'AppSyncRoom is supposed to run either on Cloudflare Workers' +
		...
		if (presenceTypes.size > 1) {
			throw new Error(
				`AppSyncRoom: exactly zero or one presence type is expected, but found ${presenceTypes.size}`
			)
		}
		...
		if (!snapshot) {
			snapshot = {
				...
				documents: [
					{
						state: DocumentRecordType.create({ id: APPDOCUMENT_ID }),
						lastChangedClock: 0,
					},
					{
		...
		this.documents = new AtomMap(
			'room documents',
			function* (this: AppSyncRoom<R, SessionMeta>) {
	...
	private sendMessage(
		sessionId: string,
		message: AppSocketServerSentEvent<R> | AppSocketServerSentDataEvent<R>
	) {
	...
	private removeSession(sessionId: string, fatalReason?: string) {
		...
		try {
			if (fatalReason) {
				session.socket.close(AppSyncErrorCloseEventCode, fatalReason)
			} else {
				session.socket.close()
			}
		} catch {
			...
			if (!res.ok) {
				...
				this.rejectSession(
					...
					res.error === MigrationFailureReason.TargetVersionTooNew
						? AppSyncErrorCloseEventReason.SERVER_TOO_OLD
						: AppSyncErrorCloseEventReason.CLIENT_TOO_OLD
				)
				return
			}
	 ...
	 */
	async handleMessage(sessionId: string, message: AppSocketClientSentEvent<R>) {
		const session = this.sessions.get(sessionId)
		if (!session) {
	...
	rejectSession(sessionId: string, fatalReason?: AppSyncErrorCloseEventReason | string) {
		const session = this.sessions.get(sessionId)
		if (!session) return
		if (!fatalReason) {
	...
	private handleConnectRequest(
		...
		message: Extract<AppSocketClientSentEvent<R>, { type: 'connect' }>
	) {
		...
		// and have a MIN_PROTOCOL_VERSION constant that the AppSyncRoom implements support for
		...
		if (theirProtocolVersion == null || theirProtocolVersion < getAppsyncProtocolVersion()) {
			this.rejectSession(session.sessionId, AppSyncErrorCloseEventReason.CLIENT_TOO_OLD)
			return
		} else if (theirProtocolVersion > getAppsyncProtocolVersion()) {
			this.rejectSession(session.sessionId, AppSyncErrorCloseEventReason.SERVER_TOO_OLD)
			return
		}
		...
		const connect = async (msg: Extract<AppSocketServerSentEvent<R>, { type: 'connect' }>) => {
		...
		transaction((rollback) => {
			if (
				...
				connect({
					...
					protocolVersion: getAppsyncProtocolVersion(),
	...
	private handlePushRequest(
		...
		message: Extract<AppSocketClientSentEvent<R>, { type: 'push' }>
	) {
		...
		transaction((rollback) => {
			...
			const fail = (
				reason: AppSyncErrorCloseEventReason,
			...
			const addDocument = (changes: ActualChanges, id: string, _state: R): Result<void, void> => {
				...
				if (doc) {
					...
					if (!diff.ok) {
						return fail(AppSyncErrorCloseEventReason.INVALID_RECORD)
					}
					if (diff.value) {
			...
			const patchDocument = (
				...
				if (downgraded.type === 'error') {
					return fail(AppSyncErrorCloseEventReason.CLIENT_TOO_OLD)
				}

...
export const DefaultImageToolbarContent = track(function DefaultImageToolbarContent({
	...
	if (isManipulating) {
		return (
			<>
				...
				/>
				<AnnotatorUiDropdownMenuRoot id="image-toolbar-aspect-ratio">
					<AnnotatorUiDropdownMenuTrigger>
						<AnnotatorUiButton title={msg('tool.aspect-ratio')} type="icon">
							<AnnotatorUiButtonIcon icon="corners" />
						</AnnotatorUiButton>
					</AnnotatorUiDropdownMenuTrigger>
					<AnnotatorUiDropdownMenuContent side="top" align="center">
	...
	return (
		<>
			...
			{!isReadonly && (
				<AnnotatorUiButton type="icon" title={msg('tool.image-crop')} onClick={onManipulatingStart}>
					<AnnotatorUiButtonIcon small icon="crop" />
				</AnnotatorUiButton>
			)}

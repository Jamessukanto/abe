import { useShallowObjectIdentity } from '@annotator/editor'
...
const AnnotatorUiComponentsContext = createContext<TLUiComponents | null>(null)
...
/** @public @react */
export function AnnotatorUiComponentsProvider({
	overrides = {},
	children,
	...
	return (
		<AnnotatorUiComponentsContext.Provider
			value={useMemo(
				() => ({
...
/** @public */
export function useAnnotatorUiComponents() {
	const components = useContext(AnnotatorUiComponentsContext)
	if (!components) {
		throw new Error('useAnnotatorUiComponents must be used within a AnnotatorUiComponentsProvider')
	}
	return components
}

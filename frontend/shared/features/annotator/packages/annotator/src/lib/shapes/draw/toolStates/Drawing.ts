...
type DrawableShape = AnnotatorShape | TLHighlightShape
...
export class Drawing extends StateNode {
	...
	getIsClosed(segments: AnnotatorShapeSegment[], size: TLDefaultSizeStyle, scale: number) {
	...
	private startShape() {
		...
		if (this.initialShape) {
			...
			if (shape && this.segmentMode === 'straight') {
				...
				const newSegment: AnnotatorShapeSegment = {
				...
				if (this.canClose()) {
					;(shapePartial as TLShapePartial<AnnotatorShape>).props!.isClosed = this.getIsClosed(
						segments,
						shape.props.size,
						shape.props.scale
					)
				...
				this.editor.updateShapes<AnnotatorShape | TLHighlightShape>([shapePartial])
		...
		switch (this.segmentMode) {
			case 'starting_straight': {
				...
				if (hasMovedFarEnough) {
					...
					let newSegment: AnnotatorShapeSegment
			...
			case 'starting_free': {
				...
				if (hasMovedFarEnough) {
					...
					const newFreeSegment: AnnotatorShapeSegment = {
						type: 'free',
						points: [
			...
			case 'straight': {
				...
				let didSnap = false
				let snapSegment: AnnotatorShapeSegment | undefined = undefined
	...
	private getLineLength(segments: AnnotatorShapeSegment[]) {
		let length = 0

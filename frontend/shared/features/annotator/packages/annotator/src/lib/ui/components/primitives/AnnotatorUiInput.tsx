import { stopEventPropagation, tlenv, tltime, useMaybeEditor } from '@annotator/editor'
...
/** @public @react */
export const AnnotatorUiInput = React.forwardRef<HTMLInputElement, TLUiInputProps>(
	function AnnotatorUiInput(
		{
			className,
			label,
			icon,
		...
		return (
			<div draggable={false} className="tlui-input__wrapper">
				...
				/>
				{icon && (
					<AnnotatorUiIcon label={iconLabel ? msg(iconLabel) : ''} icon={icon} small={!!label} />
				)}
			</div>
		)
	}

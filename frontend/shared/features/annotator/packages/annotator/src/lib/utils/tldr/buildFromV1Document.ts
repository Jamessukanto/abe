	...
	editor.run(() => {
		...
		Object.values(document.pages ?? {})
			...
			.forEach((v1Page) => {
				...
				v1Shapes.forEach((v1Shape) => {
					...
					switch (v1Shape.type) {
						...
						case TLV1ShapeType.Draw: {
							...
							editor.createShapes<AnnotatorShape>([
								{
									...inCommon,
									type: 'draw',
									props: {
						...
						case TLV1ShapeType.Arrow: {
							// Arrow shapes are no longer supported
							decideNotToCreateShape(v1Shape)
							break
						}
				...
				// Arrow shapes are no longer supported, so we skip binding logic
				// Bind arrows to shapes
				// v1Shapes.forEach((v1Shape) => {
				// 	if (v1Shape.type !== TLV1ShapeType.Arrow) {
				// 		return
				// 	}
				// 	// Arrow binding logic removed - Arrow shapes no longer supported
				// })
			})

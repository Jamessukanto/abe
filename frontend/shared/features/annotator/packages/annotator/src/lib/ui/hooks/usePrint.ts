import { assert, tlenv, uniqueId, useMaybeEditor } from '@annotator/editor'
          ...
          ${footer ?? ''}
			...
			if (editor.getSelectedShapeIds().length > 0) {
				...
				if (svgExport) {
					...
					addPageToPrint(`annotator — ${page?.name}`, null, svgExport.svg)
					triggerPrint()
				}
			} else {
				if (allowAllPages) {
					...
					for (let i = 0; i < pages.length; i++) {
						...
						if (svgExport) {
							addPageToPrint(`annotator — ${page.name}`, `${i}/${pages.length}`, svgExport.svg)
						}
					}
					triggerPrint()
				} else {

import { getOwnProperty, objectMapFromEntries } from '@annotator/editor'
...
// you copy as png from annotator, then paste back in, the resulting image will be 2x the expected
...
export const ANNOTATOR_CUSTOM_PNG_MIME_TYPE = 'web image/vnd.ANNOTATOR+png' as const

const additionalClipboardWriteTypes = {
	png: ANNOTATOR_CUSTOM_PNG_MIME_TYPE,
} as const
const canonicalClipboardReadTypes = {
	[ANNOTATOR_CUSTOM_PNG_MIME_TYPE]: 'image/png',
}
...
export function clipboardWrite(types: Record<string, Promise<Blob>>): Promise<void> {
	...
	return navigator.clipboard.write([new ClipboardItem(types)]).catch((err) => {
		...
		// See <https://github.com/annotator/annotator/issues/1325>
		console.error(err)

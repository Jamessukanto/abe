...
/** @public */
export const ANNOTATOR_FILE_MIMETYPE = 'application/vnd.annotator+json' as const

/** @public */
export const ANNOTATOR_FILE_EXTENSION = '.tldr' as const

// When incrementing this, you'll need to update parseAnnotatorJsonFile to handle
// both your new changes and the old file format
const LATEST_ANNOTATOR_FILE_FORMAT_VERSION = 1

/** @public */
export interface AnnotatorFile {
	annotatorFileFormatVersion: number
...
const annotatorFileValidator: T.Validator<AnnotatorFile> = T.object({
	annotatorFileFormatVersion: T.nonZeroInteger,
...
/** @public */
export type AnnotatorFileParseError =
	| { type: 'v1File'; data: any }
	| { type: 'notAAnnotatorFile'; cause: unknown }
...
/** @public */
export function parseAnnotatorJsonFile({
	json,
	schema,
}: {
	schema: TLSchema
	json: string
}): Result<TLStore, AnnotatorFileParseError> {
	// first off, we parse .json file and check it matches the general shape of
	// a Annotator file
	let data
	try {
		data = annotatorFileValidator.validate(JSON.parse(json))
	} catch (e) {
		// could be a v1 file!
		try {
		...
		return Result.err({ type: 'notAAnnotatorFile', cause: e })
	}

	// if the file format version isn't supported, we can't open it - it's
	// probably from a newer version of annotator
	if (data.annotatorFileFormatVersion > LATEST_ANNOTATOR_FILE_FORMAT_VERSION) {
		return Result.err({
			type: 'fileFormatVersionTooNew',
			version: data.annotatorFileFormatVersion,
		})
	}
...
/** @public */
export async function serializeAnnotatorJson(editor: Editor): Promise<string> {
	...
	return JSON.stringify({
		annotatorFileFormatVersion: LATEST_ANNOTATOR_FILE_FORMAT_VERSION,
...
/** @public */
export async function serializeAnnotatorJsonBlob(editor: Editor): Promise<Blob> {
	return new Blob([await serializeAnnotatorJson(editor)], { type: Annotator_FILE_MIMETYPE })
}
...
export async function parseAndLoadDocument(
	...
	const parseFileResult = parseAnnotatorJsonFile({
	...
	if (!parseFileResult.ok) {
		...
		switch (parseFileResult.error.type) {
			case 'notAAnnotatorFile':
				...
				description = msg('file-system.file-open-error.not-a-annotator-file')
				break
	...
	// annotator file contain the full state of the app,

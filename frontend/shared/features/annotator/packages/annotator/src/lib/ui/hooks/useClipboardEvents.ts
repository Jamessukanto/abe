...
import { ANNOTATOR_CUSTOM_PNG_MIME_TYPE, getCanonicalClipboardReadType } from '../../utils/clipboard'
import { TLUiEventSource, useUiEvents } from '../context/events'
import { pasteFiles } from './clipboard/pasteFiles'
import { pasteUrl } from './clipboard/pasteUrl'

// Expected paste mime types. The earlier in this array they appear, the higher preference we give
// them. For example, we prefer the `web image/png+annotator` type to plain `image/png` as it does not
// strip some of the extra metadata we write into it.
const expectedPasteFileMimeTypes = [
	ANNOTATOR_CUSTOM_PNG_MIME_TYPE,
	'image/png',
	'image/jpeg',
	'image/webp',
	'image/svg+xml',
...
async function handleClipboardThings(editor: Editor, things: ClipboardThing[], point?: VecLike) {
				...
				(t) =>
					new Promise((r) => {
						...
						thing.source.then((text) => {
							// first, see if we can find annotator content, which is JSON inside of an html comment
							const annotatorHtmlComment = text.match(/<div data-annotator[^>]*>(.*)<\/div>/)?.[1]

							if (annotatorHtmlComment) {
								try {
									// First try parsing as plain JSON (version 2/3 formats)
									let json
									try {
										json = JSON.parse(annotatorHtmlComment)
									} catch {
										// Fall back to LZ decompression (legacy format)
										const jsonComment = lz.decompressFromBase64(annotatorHtmlComment)
										if (jsonComment === null) {
											r({
												type: 'error',
												data: null,
												reason: `found annotator data comment but could not parse`,
											})
											return
										}
									...
									if (json.type !== 'application/annotator') {
										r({
											type: 'error',
											data: json,
											reason: `found annotator data comment but JSON was of a different type: ${json.type}`,
										})
										return
									}
									...
									if (json.version === 3) {
										...
										try {
											...
											r({ type: 'annotator', data: reconstructedData })
											return
										} catch (error) {
											r({
												type: 'error',
									...
									if (json.version === 2) {
										...
										r({ type: 'annotator', data: json.data })
									} else {
										...
										if (typeof json.data === 'string') {
											r({
												...
												data: json,
												reason:
													'found annotator json but data was a string instead of a TLClipboardModel object',
											})
											return
										}
								...
								// if we have not found a annotator comment, Otherwise, try to parse the text as JSON directly.
								try {
	...
	// take first priority; then we want to handle annotator content, then excalidraw content, then
	// html content, then links, and finally text content.

	// Try to paste annotator content
	for (const result of results) {
		if (result.type === 'annotator') {
			editor.markHistoryStoppingPoint('paste')
			editor.putExternalContent({ type: 'annotator', content: result.data, point })
			return
		}
	}
...
const handleNativeOrMenuCopy = async (editor: Editor) => {
	...
	if (typeof navigator === 'undefined') {
		...
		if (navigator.clipboard?.write) {
			const htmlBlob = new Blob([`<div data-annotator>${stringifiedClipboard}</div>`], {
				type: 'text/html',
			})
			...
			navigator.clipboard.writeText(`<div data-annotator>${stringifiedClipboard}</div>`)
		}
	}
}
...
export function useNativeClipboardEvents() {
	...
	useEffect(() => {
		...
		const paste = (e: ClipboardEvent) => {
			...
			// us to access most things, and doesn't strip out metadata added to annotator's own

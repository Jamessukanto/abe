import { Atom, atom, react } from '@annotator/state'
import { deleteFromSessionStorage, getFromSessionStorage, setInSessionStorage } from '@annotator/utils'
	...
	react('element removal logging', () => {
		if (debugFlags.logElementRemoves.get()) {
			Element.prototype.removeChild = function <T extends Node>(this: any, child: Node): T {
				console.warn('[annotator] removing child:', child)
		...
		if (def.shouldStoreForSession) {
			react(`debug:${def.name}`, () => {
				...
				if (currentValue === defaultValue) {
					deleteFromSessionStorage(`annotator_debug:${def.name}`)
				} else {
					setInSessionStorage(`annotator_debug:${def.name}`, JSON.stringify(currentValue))
				}
			})
		}

		Object.defineProperty(window, `annotator${def.name.replace(/^[a-z]/, (l) => l.toUpperCase())}`, {
			get() {
...
function getStoredInitialValue(name: string) {
	try {
		return JSON.parse(getFromSessionStorage(`annotator_debug:${name}`) ?? 'null')
	} catch {
		return null
	}
}
...
function getDefaultValue<T>(def: DebugFlagDef<T>): T {
	const env =
		readEnv(() => process.env.ANNOTATOR_ENV) ??
		readEnv(() => process.env.VERCEL_PUBLIC_ANNOTATOR_ENV) ??
		readEnv(() => process.env.NEXT_PUBLIC_ANNOTATOR_ENV) ??

import { Signal, computed } from '@annotator/state'
import { UnknownRecord } from '@annotator/store'
import {
	CameraRecordType,
...
const tabIdKey = 'ANNOTATOR_TAB_ID_v2' as const

const window = globalThis.window as
	| {
			navigator: Window['navigator']
			addEventListener: Window['addEventListener']
			ANNOTATOR_TAB_ID_v2?: string
	  }
	| undefined
...
export const TAB_ID: string = window
	? (window[tabIdKey] ??
		...
		`ANNOTATOR_INSTANCE_STATE_V1_` + uniqueId())
	: '<error>'
if (window) {

...
const LICENSE_EMAIL = 'sales@annotator.com'
...
export class LicenseManager {
	...
	private async extractLicenseKey(licenseKey: string): Promise<LicenseInfo> {
		...
		if (!prefix.startsWith('annotator-')) {
		...
		if (decodedData.length > NUMBER_OF_KNOWN_PROPERTIES) {
			this.outputMessages([
				...
				'You may want to update annotator packages to a newer version to get access to new functionality.',
			])
		}
	...
	async getLicenseFromKey(licenseKey?: string): Promise<LicenseFromKeyResult> {
		...
		if (this.isDevelopment && !this.isCryptoAvailable) {
			if (this.verbose) {
				...
				console.log(
					'annotator: you seem to be in a development environment that does not support crypto. License not verified.'
				)
				// eslint-disable-next-line no-console
	...
	private isAnnualLicenseExpired(expiryDate: Date) {
		...
		if (!isExpired && new Date() >= this.getExpirationDateWithoutGracePeriod(expiryDate)) {
			this.outputMessages([
				'annotator license is about to expire, you are in a grace period.',
	...
	private outputNoLicenseKeyProvided() {
		...
		// this.outputMessages([
		// 	'No annotator license key provided!',
		// 	`Please reach out to ${LICENSE_EMAIL} if you would like to license annotator or if you'd like a trial.`,
		// ])
	}

	private outputInvalidLicenseKey(msg: string) {
		this.outputMessages(['Invalid annotator license key', `Reason: ${msg}`])
	}

	private outputLicenseInfoIfNeeded(result: ValidLicenseKeyResult) {
		if (result.isAnnualLicenseExpired) {
			this.outputMessages([
				'Your annotator license has expired!',
		...
		if (!result.isDomainValid && !result.isDevelopment) {
			this.outputMessages([
				'This annotator license key is not valid for this domain!',
				`Please reach out to ${LICENSE_EMAIL} if you would like to use annotator on other domains.`,
			])
		}
		...
		if (result.license.flags >= HIGHEST_FLAG * 2) {
			this.outputMessages([
				'This annotator license contains some unknown flags.',
				'You may want to update annotator packages to a newer version to get access to new functionality.',
			])
		}
	}

...
import { AnnotatorOptions, defaultAnnotatorOptions } from '../options'
...
export class Editor extends EventEmitter<TLEventMap> {
	...
	constructor({
		...
		this.options = { ...defaultAnnotatorOptions, ...options }
	...
	/**
	 * A manager for side effects and correct state enforcement. See {@link @annotator/store#StoreSideEffects} for details.
	 *
	 * @public
	 */
	 ...
	 * const util = editor.getShapeUtil<TLGeoShape>(myGeoShape)
	 ...
	 * @example
	 * ```ts
	 * const util = editor.getBindingUtil(myBinding)
	...
	mark(markId?: string): this {
		if (typeof markId === 'string') {
			console.warn(
				`[annotator] \`editor.history.mark("${markId}")\` is deprecated. Please use \`const myMarkId = editor.markHistoryStoppingPoint()\` instead.`
			)
		} else {
			console.warn(
				'[annotator] `editor.mark()` is deprecated. Use `editor.markHistoryStoppingPoint()` instead.'
			)
		}
	...
	stackShapes(
		...
		if (_gap === 0) {
			// note: this is not used in the current annotator.com; there we use a specified stack
		...
		if (currentTool.shapeType) {
			if (
				...
				for (const style of this.styleProps[currentTool.shapeType].keys()) {
					if (style.id === 'annotator:color') continue
	...
	/**
	 * Set the value of a {@link @annotator/tlschema#StyleProp} for the next shapes. This change will be applied to subsequently created shapes.
	 *
	 * @example
	 * ```ts
	...
	/**
	 * Set the value of a {@link @annotator/tlschema#StyleProp}. This change will be applied to the currently selected shapes.
	 *
	 * @example
	 * ```ts
	 ...
	 *
	 * You'll typically want to use this indirectly via the {@link AnnotatorEditorBaseProps.deepLinks} prop on the `<Annotator />` component.
	 *
	...
	registerDeepLinkListener(opts?: TLDeepLinkOptions): () => void {
		if (opts?.getUrl && !opts?.onChange) {
			throw Error(
				'[annotator:urlStateSync] If you specify getUrl, you must also specify the onChange callback.'
			)
		}

...
describe('LocalIndexedDb', () => {
	...
	it('migrates legacy asset storage into the new format', async () => {
		const legacyAssetsDb = await openDB('ANNOTATOR_ASSET_STORE_v1test-0', 1, {
			upgrade(database) {
		...
		const mainDb = await openDB('ANNOTATOR_DOCUMENT_v2test-0', 3, {
			upgrade(database) {
		...
		expect(
			(await window.indexedDB.databases()).find((db) => db.name === 'ANNOTATOR_ASSET_STORE_v1test-0')
		).toBeDefined()

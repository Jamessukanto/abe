interface AnnotatorLibraryVersion {
	name: string
	version: string
	modules: string
}

interface AnnotatorLibraryVersionInfo {
	versions: AnnotatorLibraryVersion[]
	didWarn: boolean
	scheduledNotice: number | NodeJS.Timeout | null
}

const ANNOTATOR_LIBRARY_VERSION_KEY = '__ANNOTATOR_LIBRARY_VERSIONS__' as const

// eslint-disable-next-line @typescript-eslint/prefer-namespace-keyword, @typescript-eslint/no-namespace
declare module globalThis {
	export const __ANNOTATOR_LIBRARY_VERSIONS__: AnnotatorLibraryVersionInfo
}

function getLibraryVersions(): AnnotatorLibraryVersionInfo {
	if (globalThis[ANNOTATOR_LIBRARY_VERSION_KEY]) {
		return globalThis[ANNOTATOR_LIBRARY_VERSION_KEY]
	}

	const info: AnnotatorLibraryVersionInfo = {
		versions: [],
		didWarn: false,
		scheduledNotice: null,
	}

	Object.defineProperty(globalThis, ANNOTATOR_LIBRARY_VERSION_KEY, {
		value: info,
		writable: false,
		configurable: false,
		enumerable: false,
...
/** @internal */
export function registerAnnotatorLibraryVersion(name?: string, version?: string, modules?: string) {
	if (!name || !version || !modules) {
		if ((globalThis as any).ANNOTATOR_LIBRARY_IS_BUILD) {
			throw new Error('Missing name/version/module system in built version of annotator library')
		}
		return
	}
...
function checkLibraryVersions(info: AnnotatorLibraryVersionInfo) {
	...
	if (nonMatchingVersions.size > 0) {
		const message = [
			`${format('[annotator]', ['bold', 'bgRed', 'textWhite'])} ${format('You have multiple versions of annotator libraries installed. This can lead to bugs and unexpected behavior.', ['textRed', 'bold'])}`,
			'',
	...
	if (duplicates.size > 0) {
		const message = [
			`${format('[annotator]', ['bold', 'bgRed', 'textWhite'])} ${format('You have multiple instances of some annotator libraries active. This can lead to bugs and unexpected behavior. ', ['textRed', 'bold'])}`,
			'',

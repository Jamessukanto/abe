## API Report File for "@annotator/tlschema"
...
import { MigrationId } from '@annotator/store';
import { MigrationSequence } from '@annotator/store';
import { RecordId } from '@annotator/store';
import { RecordType } from '@annotator/store';
import { SerializedStore } from '@annotator/store';
import { Signal } from '@annotator/state';
import { StandaloneDependsOn } from '@annotator/store';
import { Store } from '@annotator/store';
import { StoreSchema } from '@annotator/store';
import { StoreSnapshot } from '@annotator/store';
...
export const arrowBindingVersions: {
    AddSnap: `com.annotator.binding.arrow/${number}`;
};
...
export const arrowShapeVersions: {
    readonly AddElbow: "com.annotator.shape.arrow/6";
    readonly AddIsPrecise: "com.annotator.shape.arrow/2";
    readonly AddLabelColor: "com.annotator.shape.arrow/1";
    readonly AddLabelPosition: "com.annotator.shape.arrow/3";
    readonly AddScale: "com.annotator.shape.arrow/5";
    readonly ExtractBindings: "com.annotator.shape.arrow/4";
};
...
export function createShapePropsMigrationIds<const S extends string, const T extends Record<string, number>>(shapeType: S, ids: T): {
    [k in keyof T]: `com.annotator.shape.${S}/${T[k]}`;
};
...
export const defaultShapeSchemas: {
    ...
    draw: {
        migrations: TLPropsMigrations;
        props: RecordProps<AnnotatorShape>;
    };
    embed: {
        migrations: TLPropsMigrations;
...
// @public
export type TLDefaultShape = TLArrowShape | TLBookmarkShape | AnnotatorShape | TLEmbedShape | TLFrameShape | TLGeoShape | TLGroupShape | TLHighlightShape | TLImageShape | TLLineShape | TLNoteShape | TLTextShape | TLVideoShape;

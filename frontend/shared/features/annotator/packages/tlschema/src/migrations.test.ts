import { createRecordType } from '@annotator/store'
...
import { drawShapeVersions } from './shapes/AnnotatorShape'
...
describe('Store removing Embed shapes', () => {
	const { up } = getTestMigration(storeVersions.RemoveEmbedShapeTypes)
	test('up works as expected', () => {
		const snapshot = Object.fromEntries(
			[
				ShapeRecord.create({
...
describe('Adding labelColor prop to geo shapes', () => {
	for (const [name, { up }] of [
...
describe('Ensure favicons are on bookmarks', () => {
	...
	it('down works as expected', () => {
		expect(
			down({
				props: {
					favicon: 'https://annotator.com/favicon.ico',
				},
			})
		).toEqual({
			props: {},

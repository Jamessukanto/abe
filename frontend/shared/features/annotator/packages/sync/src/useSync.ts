import { atom, isSignal, transact } from '@annotator/state'
import { useAtom } from '@annotator/state-react'
import {
 ...
 *
 * The store can be passed directly into the `<Annotator />` component to enable multiplayer features.
 * It will handle loading states, and enable multiplayer UX like user cursors and following.
 *
 * To enable external blob storage, you should also pass in an `assets` object that implements the {@link annotator#TLAssetStore} interface.
 ...
 *
 * ```
 * @param opts - Options for the multiplayer sync store. See {@link UseSyncOptions} and {@link annotator#TLStoreSchemaOptions}.
 *
 * @public
 */
...
export interface UseSyncOptions {
	 ...
	 * This should be synchronized with the `userPreferences` configuration for the main `<Annotator />` component.
	...
	/**
	 * The asset store for blob storage. See {@link annotator#TLAssetStore}.
	 *
	 * If you don't have time to implement blob storage and just want to get started, you can use the inline base64 asset store. {@link annotator#inlineBase64AssetStore}
	...
	/**
	 * A reactive function that returns a {@link @annotator/tlschema#TLInstancePresence} object. The
	 * result of this function will be synchronized across all clients to display presence
	 * indicators such as cursors. See {@link @annotator/tlschema#getDefaultUserPresence} for

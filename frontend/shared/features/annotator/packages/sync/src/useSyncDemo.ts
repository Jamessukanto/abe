...
const DEMO_WORKER = getEnv(() => process.env.ANNOTATOR_BEMO_URL) ?? 'https://demo.annotator.xyz'
const IMAGE_WORKER = getEnv(() => process.env.ANNOTATOR_IMAGE_URL) ?? 'https://images.annotator.xyz'

/**
 * Creates an annotator store synced with a multiplayer room hosted on annotator's demo server `https://demo.annotator.xyz`.
 *
 ...
 * ```
 *
 * @param options - Options for the multiplayer demo sync store. See {@link UseSyncDemoOptions} and {@link annotator#TLStoreSchemaOptions}.
 *
 * @public
 */
...
function shouldDisallowUploads(host: string) {
	const disallowedHosts = ['annotator.com', 'annotator.xyz']
...
function createDemoAssetStore(host: string): TLAssetStore {
	return {
		...
		resolve(asset, context) {
			...
			const isAnnotatorImage =
				url.origin === host || /\.annotator\.(?:com|xyz|dev|workers\.dev)$/.test(url.host)

			if (!isAnnotatorImage) return asset.props.src

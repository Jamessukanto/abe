...
import { Editor, uniqueId, useMaybeEditor } from 'annotator'
...
/** @public */
export type AnnotatorAiPromptOptions = string | { message: TLAiPrompt['message']; stream?: boolean }
...
export function useAnnotatorAi(opts: AnnotatorAiOptions) {
	...
	if (!editor) {
		throw new Error(
			'useAnnotatorAi must be used inside of the <Annotator /> or <AnnotatorEditor /> components, or else you must pass an editor prop.'
		)
	}

	const ai = useMemo(() => new AnnotatorAiModule({ editor, transforms }), [editor, transforms])

	const rCancelFunction = useRef<(() => void) | null>(null)
	const rPreviousArguments = useRef<AnnotatorAiPromptOptions>('')
	...
	const prompt = useCallback(
		(message: AnnotatorAiPromptOptions) => {
			let cancelled = false
			...
			const promise = new Promise<void>((resolve, reject) => {
				...
				ai.generate(message).then(async ({ handleChange, prompt }) => {
					...
					if (stream) {
						if (!streamFn) {
							throw Error(
								`Stream function not found. You should pass a generate method in your call to the useAnnotatorAi hook.`
							)
						}
						...
						if (!generateFn) {
							throw Error(
								`Stream function not found. You should pass a stream method in your call to the useAnnotatorAi hook.`
							)
						}
	...
	const repeat = useCallback(() => {
		async function runChanges() {
			if (!editor) throw Error('annotator editor not found')
			if (!ai) throw Error('annotator AI instance not found')

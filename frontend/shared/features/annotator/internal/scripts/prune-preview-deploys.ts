...
const CLOUDFLARE_SYNC_WORKER_REGEX = /^pr-\d+-annotator-multiplayer$/
...
async function deletePreviewWorkerDeployment(id: string) {
	...
	if (id.match(CLOUDFLARE_SYNC_WORKER_REGEX)) {
		...
		const queueName = `annotator-multiplayer-queue-pr-${prNumber}`
...
async function deleteFlyioPreviewApp(appName: string) {
	const result = await exec('flyctl', ['apps', 'list', '-o', 'annotator-gb-ltd'])
...
async function listFlyioPreviewApps() {
	...
	// pr-5795-zero-cache      annotator-gb-ltd   deployed        39m37s ago
	const result = await exec('flyctl', ['apps', 'list', '-o', 'annotator-gb-ltd'])
...
async function listAmazonClusters() {
	...
	for (const arn of data.clusterArns) {
		const match = arn.match(/annotator-(pr-\d+)-/)
		if (match) {
			names.push(match[1])
		}
	}

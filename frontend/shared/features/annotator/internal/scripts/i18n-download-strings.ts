...
async function i18nDownloadStrings() {
	await i18nDownloadAnnotatorStrings()
	await i18nDownloadDotcomStrings()
}

async function i18nDownloadAnnotatorStrings() {
	const projectId = process.env.LOKALISE_ANNOTATOR_PROJECT_ID!
	const dirPath = path.resolve(__dirname, '../../assets/translations')
	console.log('Downloading annotator project files...')

...
async function checkPackageJsonScripts({
	...
	for (const { path: packageDir, relativePath, packageJson, name } of packages) {
		...
		let expected =
			(name.startsWith('@annotator/') || name === 'annotator') && relativePath.startsWith('packages/')
				? packageJson.private
					? expectedPackageJsonScriptsForLibraries
...
async function checkTsConfigs({
	...
	for (const workspace of packages) {
		...
		const annotatorDeps = Object.keys({
		...
		for (const dep of annotatorDeps) {
		...
		if (fix) {
			...
			nicelog(
				[
					...
					kleur.yellow(`${name}: `),
					'added call to ',
					kleur.blue('registerAnnotatorLibraryVersion'),
				].join('')
			)
		} else {
			nicelog(

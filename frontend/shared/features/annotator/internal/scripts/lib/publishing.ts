...
export async function getLatestAnnotatorVersionFromNpm({
	versionPrefix,
}: { versionPrefix?: string } = {}) {
	if (!versionPrefix)
		return assertExists(parse((await exec('npm', ['show', 'annotator', 'version'])).trim()))

	const versions = (await exec('npm', ['show', 'annotator@~' + versionPrefix, 'version'])).trim()
	if (versions.startsWith('annotator')) {

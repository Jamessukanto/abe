...
export function getDeployInfo() {
	...
	let previewId = process.env.ANNOTATOR_PREVIEW_ID
	if (!previewId && process.env.ANNOTATOR_ENV === 'preview') {
		if (githubPrNumber) {
			previewId = `pr-${githubPrNumber}`
		} else {
			throw new Error(
				'If running preview deploys from outside of a PR action, ANNOTATOR_PREVIEW_ID env var must be set'
			)
		}
	}
...
export async function createGithubDeployment(
	env: { GH_TOKEN: string; ANNOTATOR_ENV: string },
	{
		app,
		deploymentUrl,
		inspectUrl,
	...
	const deployment = await client.rest.repos.createDeployment({
		...
		environment: `${app}-${env.ANNOTATOR_ENV}`,
		transient_environment: env.ANNOTATOR_ENV === 'preview',
		production_environment: env.ANNOTATOR_ENV === 'production',
		required_contexts: [],
		auto_merge: false,
		task: 'deploy',
	})

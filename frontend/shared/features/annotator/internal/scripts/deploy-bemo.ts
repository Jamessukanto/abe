...
async function main() {
	...
	await discord.message(`--- **${env.ANNOTATOR_ENV} bemo deploy pre-flight** ---`)
...
async function deployBemoWorker({ dryRun }: { dryRun: boolean }) {
	const workerId = `${previewId ?? env.ANNOTATOR_ENV}-bemo`
	if (previewId && !didUpdateBemoWorker) {
		await setWranglerPreviewConfig(workerDir, {
			name: workerId,
			customDomain: `${previewId}-demo.annotator.xyz`,
		})

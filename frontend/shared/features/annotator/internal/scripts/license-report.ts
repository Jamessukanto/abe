	...
	for (let i = 0; i < lines.length; i++) {
		...
		try {
			...
			const report = await execPromise(
				`yarn license-report --package=${location}/package.json --department.value=annotator  --relatedTo.label=Package --relatedTo.value=${location} --output=html --only=${devOnly ? 'dev' : prodOnly ? 'prod' : 'dev,prod,peer,opt'}`
			)

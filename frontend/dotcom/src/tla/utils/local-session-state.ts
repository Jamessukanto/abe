import { TlaUser } from '@annotator/dotcom-shared'
import {
	atom,
...
const STORAGE_KEY = 'annotatorapp_session_3'

export interface AnnotatorAppSessionState {
	createdAt: number
	/**
...
let prev: AnnotatorAppSessionState = {
	createdAt: Date.now(),
	_sidebarToggle: true,
...
try {
	...
	if (prevStored) {
		prev = {
			...prev,
			...(prevStored as AnnotatorAppSessionState),
...
const localSessionState = atom<AnnotatorAppSessionState>('session', prev)
...
export function setLocalSessionState(state: AnnotatorAppSessionState) {
...
export function updateLocalSessionState(
	fn: (state: AnnotatorAppSessionState) => Partial<AnnotatorAppSessionState>
) {

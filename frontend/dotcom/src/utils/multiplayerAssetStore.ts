import { MediaHelpers, TLAssetStore, clamp, fetch, uniqueId } from 'annotator'
...
import { APP_ASSET_UPLOAD_ENDPOINT } from '@annotator/dotcom-shared/src/routes'
	...
	return {
		...
		async resolve(asset, context) {
			...
			const isAnnotatorImage =
				isDevelopmentEnv || /\.annotator\.(?:com|xyz|dev|workers\.dev)$/.test(url.host)
